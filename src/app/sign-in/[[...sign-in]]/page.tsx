...
export default function SignInPage() {
    ...
    // Timeout: if stuck redirecting for >6 seconds, show manual link
    useEffect(() => {
        if (isLoaded && isSignedIn) {
                ...
                <div className="min-h-screen flex items-center justify-center bg-black">
                    <div className="flex flex-col items-center gap-4 max-w-md text-center p-6">
                        ...
                        <p className="text-zinc-400 text-sm">
                            Tu sesión es válida pero la redirección falló. Esto suele ocurrir cuando el reloj del sistema no está sincronizado.
                        </p>
                        <div className="flex flex-col gap-3 w-full">
                            <a
                                href="/dashboard"
                                className="bg-purple-600 hover:bg-purple-500 text-white font-semibold py-3 px-6 rounded-full text-center transition-colors"
                            >
                                Ir al Dashboard manualmente
                            </a>
                            <p className="text-zinc-600 text-xs">
                                💡 Tip: Sincroniza la hora de tu sistema en Configuración → Fecha y hora → Activar &quot;Ajustar hora automáticamente&quot;
                            </p>
                        </div>
                    </div>
                </div>
