...
export function ApiConfigTab() {
    ...
    const providers = [
        {
            id: "gemini",
            name: "Google Gemini",
            desc: "Usado para extracción de datos y ángulos comerciales. Plan gratuito de 15 RPM.",
            link: "https://aistudio.google.com/app/apikey"
        },
        {
            id: "groq",
            name: "Groq",
            desc: "Motor ultrarrápido LLaMA-3 para copys y análisis texto. Hasta 1,000 req/día gratis.",
            link: "https://console.groq.com/keys"
        },
        {
            id: "replicate",
            name: "Replicate",
            desc: "Usado para generación de imágenes (Stable Diffusion). Requiere tarjeta o saldo inicial.",
            link: "https://replicate.com/account/api-tokens"
        },
        {
            id: "huggingface",
            name: "Hugging Face",
            desc: "Alternativa gratuita de inferencia (Inference API) para LLMs y SD. Limitado por horas.",
    ...
    const handleAddKey = (providerId: string) => {
        ...
        if (currentKeys.includes(keyVal)) {
            toast.error("Esta clave ya está agregada.");
            return;
        }
    ...
    const handleSaveAPI = async () => {
        ...
        try {
            ...
            toast.success("Claves API guardadas con éxito", { id: toastId });
        } catch (error: any) {
    ...
    const handleTestKeys = async () => {
        ...
        setIsTesting(true);
        const toastId = toast.loading("Probando conexión con proveedores...");
        try {
            // Simulando ping a rotador
            await fetch("/api/test-providers", { method: "POST", body: JSON.stringify({ providersKeys }) }).catch(() => null);
            await new Promise(r => setTimeout(r, 1500));
            toast.success("¡Pruebas exitosas! Las llaves configuradas responden correctamente.", { id: toastId });
        } catch (e) {
            toast.error("Algunas llaves fallaron. Revisa la consola.", { id: toastId });
        } finally {
            setIsTesting(false);
        }
    };
    ...
    return (
        <div className="max-w-2xl animate-in fade-in duration-300">
            <h3 className="text-2xl font-bold text-white mb-2">Conecta tus proveedores de IA (opcional)</h3>
            <p className="text-sm text-zinc-400 mb-6">
                Puedes configurar tus propias claves de múltiples proveedores gratuitos. La app rotará automáticamente entre ellas para maximizar tu disponibilidad. Si no configuras ninguna, usaremos claves globales limitadas.
            </p>
            ...
            <div className="flex gap-4 mb-6">
                ...
                >
                    {isSaving ? <Loader2 className="w-4 h-4 animate-spin mx-auto" /> : "Guardar Configuración"}
                </button>
                <button
                    ...
                    return (
                        <div key={provider.id} className="bg-[#08080A] border border-white/5 rounded-2xl p-5">
                            ...
                            <div className="space-y-2 mb-4">
                                {keys.length === 0 && (
                                    <div className="text-xs text-zinc-600 italic">No hay claves configuradas.</div>
                                )}
                                {keys.map((k, idx) => (
