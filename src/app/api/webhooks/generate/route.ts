...
export async function POST(req: Request) {
    try {
        ...
        const { creativeIds, prompt, replicateKey, generationModel } = payload;
        ...
        console.log(`[Worker] Generando con Replicate (variantes: ${creativeIds.length}, modelo: ${generationModel})...`);

        let output: string[] = [];

        if (generationModel === 'flux-pro') {
            // black-forest-labs/flux-1.1-pro 
            // Doesn't support num_outputs > 1 natively in standard API without multiple calls, or it outputs single images.
            // But we can just loop over the creativeIds and generate them in parallel using Promise.all
            console.log("[Worker] Usando Flux 1.1 Pro...");
            const promises = creativeIds.map(() =>
                replicate.run("black-forest-labs/flux-1.1-pro", {
                    input: {
                        prompt: prompt,
                        aspect_ratio: "4:5", // standard portrait
                        output_format: "png"
                    }
                })
            );

            // Replicate flux-1.1-pro returns a single ReadableStream or URL, we await all
            // Format can be a string URL per generation
            const results = await Promise.all(promises);
            // Replicate Flux 1.1 Pro returns a stream or URL string directly, make sure it's strings
            output = results.map(r => String(r));
        } else {
            // Fallback / standard is SDXL
            console.log("[Worker] Usando SDXL...");
